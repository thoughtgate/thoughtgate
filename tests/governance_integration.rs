...
use thoughtgate::config::Config;
use thoughtgate::governance::approval::MockAdapter;
...
fn test_task_state_transitions() {
    ...
    store
        .complete(
            &task.id,
            thoughtgate::governance::ToolCallResult {
...
async fn test_mock_adapter_instant_approval() {
    use chrono::Utc;
    use thoughtgate::governance::ApprovalAdapter;
    use thoughtgate::governance::ApprovalRequest;
    ...
    assert_eq!(
        ...
        thoughtgate::governance::PollDecision::Approved
    );
}
...
fn test_minimal_config_parsing() {
    ...
    assert_eq!(
        ...
        thoughtgate::config::Action::Forward
    );
...
fn test_config_with_approval_parsing() {
    ...
    assert_eq!(
        ...
        thoughtgate::config::Action::Approve
    );
    assert!(
        config
            .approval
...
fn test_config_with_deny_parsing() {
    ...
    assert_eq!(
        config.governance.rules[0].action,
        thoughtgate::config::Action::Deny
    );
...
async fn test_gate2_forward_action_passthrough() {
    ...
    use thoughtgate::transport::{McpHandler, McpHandlerConfig};
    ...
    let upstream = Arc::new(
        thoughtgate::transport::UpstreamClient::new(thoughtgate::transport::UpstreamConfig {
