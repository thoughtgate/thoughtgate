...
use thoughtgate_core::governance::engine::{ApprovalEngineConfig, TimeoutAction};
use thoughtgate_core::governance::task::{JsonRpcId, ToolCallRequest};
use thoughtgate_core::governance::{ApprovalEngine, Principal, TaskStatus, TaskStore};
use thoughtgate_core::policy::engine::CedarEngine;
use thoughtgate_core::transport::{JsonRpcResponse, McpRequest, UpstreamForwarder};
...
impl ApprovalAdapter for E2eApprovalAdapter {
    async fn post_approval_request(
        &self,
        request: &thoughtgate_core::governance::approval::ApprovalRequest,
    ...
    async fn forward(&self, request: &McpRequest) -> Result<JsonRpcResponse, ThoughtGateError> {
        ...
        *self.last_params.lock().await = request.params.as_deref().cloned();
        ...
        Ok(JsonRpcResponse {
            ...
            id: Some(thoughtgate_core::transport::JsonRpcId::Number(1)),
            result: Some(result),
            error: None,
        })
    }
