...
mod tests {
    ...
    fn test_serde_roundtrip() {
        ...
        let parsed: TestStruct = serde_saphyr::from_str(yaml).unwrap();
        assert_eq!(parsed.duration, Duration::from_secs(600));

        // Note: serialized form may differ (humantime format)
        let serialized = serde_saphyr::to_string(&parsed).unwrap();
        let reparsed: TestStruct = serde_saphyr::from_str(&serialized).unwrap();
